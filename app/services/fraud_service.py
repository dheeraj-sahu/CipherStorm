...
def run_fraud_pipeline(transaction_obj, profile_obj, txn_count: int, last_transaction_location=None, db_session=None):
    """Main fraud detection pipeline with Layer 1 (Global) + Layer 3 (Rule-based)"""
    ...
    layer3_result = rule_based_layer3_predict(transaction_obj, user_stats, distance_from_last)

    # --- Final Decision Fusion ---
    # Combine global model score with rule-based detection
    if layer3_result['is_anomaly']:
        final_score = (global_score * 0.4) + (layer3_result['confidence'] * 0.6)
    else:
        final_score = global_score
    
    # Final prediction: fraud if either global model predicts high probability OR rules are triggered
    final_prediction = int(final_score > 0.5 or layer3_result['is_anomaly'])
